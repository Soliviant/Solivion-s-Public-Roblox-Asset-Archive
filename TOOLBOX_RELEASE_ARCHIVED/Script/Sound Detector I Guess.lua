...
local function HandleSoundExplosion(descendant, dt)
	...
	local VolumeType = descendant.PlaybackLoudness * dt -- Configure What U Want It to Consider, Maybe Use PlaybackLoudness For More Accuracy
	local blastRadius = VolumeType * 50
	local soundAreaReaction = Instance.new("Explosion", descendant.Parent) -- detect things in nearby area
	...
	soundAreaReaction.Visible = false -- Mayb set it 2 tru, So u Could Debug It.
	soundAreaReaction.BlastPressure = 0 or VolumeType
	...
	if descendant:IsA("Sound") then
		...
		descendant.Played:Connect(function(ID)
			...
			while descendant.Playing and descendant.Parent do
				...
				local dt = task.wait() --delay..
				HandleSoundExplosion(descendant, dt)
			end
		end)
	end
end
